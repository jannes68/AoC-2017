...
public class Dag8 {
	...
	public static void main(String[] args) {
		...
		List<Instr> instructions = data.stream().map(r -> new Instr(r)).collect(Collectors.toList());

		instructions.forEach(i -> i.exec());

		int max = regMap.values().stream().mapToInt(v -> v).max().getAsInt();

		regMap.keySet().stream().forEach(k -> System.out.println(k + "->" + regMap.get(k)));
	...
	static int getReg(String regName) {
		if (!regMap.containsKey(regName)) {
			regMap.put(regName, Integer.valueOf(0));
		}
		return regMap.get(regName);
	}

	static void setReg(String regName, int val) {
		regMap.put(regName, val);
	}

	static class Instr {
		String instrRegister;
		InstrType instrType;
		int instrVal;
		...
		void exec() {
			if (condOp.eval(condReg, condVal)) {
				instrType.exec(instrRegister, instrVal);
				System.out.println(this.instrRegister + " " + this.instrType + " " + this.instrVal + " -> " + getReg(this.instrRegister));
			}
		}

		Instr(String row) {
			String[] parts = row.replace("if ", "").split(" ");
			int i = 0;
			this.instrRegister = parts[i++];
			this.instrType = InstrType.fromName(parts[i++]);
			this.instrVal = Integer.parseInt(parts[i++]);
			this.condReg = parts[i++];
			this.condOp = CondOp.fromName(parts[i++]);
			this.condVal = Integer.parseInt(parts[i++]);
	...
	enum InstrType {
		INC("inc", (r, v) -> r + v), DEC("dec", (r, v) -> r - v);

		String name;
		BiFunction<Integer, Integer, Integer> func;

		InstrType(String name, BiFunction<Integer, Integer, Integer> func) {
			this.name = name;
			this.func = func;
		}

		static InstrType fromName(String name) {
			for (InstrType v : values())
				if (v.name.equalsIgnoreCase(name))
					return v;
			throw new IllegalArgumentException();
		}

		void exec(String regName, Integer val) {
			setReg(regName, func.apply(getReg(regName), val));
		}
	}
	...
	enum CondOp {
		...
		CondOp(String name, BiFunction<Integer, Integer, Boolean> func) {
			this.name = name;
			this.func = func;
		}

		static CondOp fromName(String name) {
			for (CondOp v : values())
				if (v.name.equalsIgnoreCase(name))
					return v;
			throw new IllegalArgumentException();
		...
		boolean eval(String regName, Integer val) {
			return func.apply(getReg(regName), val);
		}
	}
