...
public class Day12 {
	...
	static int countNotVisitedChildren(String nodeName) {
		int count = 1; 
		visited.add(nodeName);
		List<String> childs = nodes.get(nodeName);
		for(String childName: childs) {
			if(!visited.contains(childName)) {
				count += countNotVisitedChildren(childName);
			}
		}
		return count;
	}
	
	static int assignGroup(String nodeName, Integer proposedGroup) {
		int count = 1; 
		Integer group = proposedGroup;
		nameToGroup.put(nodeName, group);
		List<String> childs = nodes.get(nodeName);
		for(String childName: childs) {
			if(!nameToGroup.keySet().contains(childName)) {
				 Integer subGroup = assignGroup(childName,group);
			} else {
				Integer existingGroup = nameToGroup.get(childName);
	...
	public static void main(String[] args) {
		...
		System.out.println( "Size of group 0" + countNotVisitedChildren("0") );
		
		nodes.keySet().stream().forEach(n -> {
...
//			);
	...
	static Map<String,List<String>> nodes =  data.stream().map(r -> r.split(" <-> ")).collect(Collectors.toMap(sr -> sr[0],sr -> (List<String>)(Arrays.asList( sr[1].replace(" ","").split(","))) ));
	static Set<String> visited = new HashSet<>();
	static Map<String,Integer> nameToGroup = new HashMap<>();
}
