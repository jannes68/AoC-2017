...
--- Part Two ---
	...
	[ ]             [ ]     [ ]
	                [ ]     [ ]
	Because all smaller delays would get you caught, the fewest number of picoseconds you would need to delay 
	to get through safely is 10.
	
	What is the fewest number of picoseconds that you need to delay the packet to pass through the firewall without being caught?	
	
	 */

	static class Layer implements Cloneable {
		private int posX;
		private int range;
		...
		public String toString() {
			return "Layer " + posX + "[ range=" + range + ", scanPos=" + scanPos + ", dir=" + dir + "]";
		}
	...
	static Map<Integer,Layer> deepCopy(Map<Integer,Layer> src) {
		return src.keySet().stream().map(k -> src.get(k).clone() ).collect(Collectors.toMap(k -> k.getPosX(), l -> l));
	}
	...
	public static void main(String[] args) {
		...
		int delay = 0;
		// Init data
		indexToLayer = data.stream().map(r -> r.split(":")).collect(Collectors.toMap(a0 -> Integer.valueOf(a0[0]),
				a1 -> new Layer(Integer.valueOf(a1[1].trim()), Integer.valueOf(a1[0]))));
		int maxPos = indexToLayer.keySet().stream().mapToInt(i -> i).max().getAsInt();
		
		Map<Integer,Layer> indexToLayerTmp;
		do {
			
			// backup state
			indexToLayerTmp = deepCopy(indexToLayer);
			// check if passage
			failed = false;
			for (int myPos = 0; myPos <= maxPos && !failed; myPos++) {
				if (indexToLayer.containsKey(myPos) && indexToLayer.get(myPos).getHit()) {
					failed = true;
				} else {
		...
		System.out.println("delay: " + delay); // 3875838
